...
class ButtonIcons(object):
    ...
    def __init__(self, image_file):
        ...
        self.filestream = IO.FileStream(
            image_file, IO.FileMode.Open, IO.FileAccess.Read
        )
    ...
    def check_icon_size(self):
        ...
        if image_size > 96:
            mlogger.warning(
                "Icon file is too large. Large icons adversely "
    ...
    def create_bitmap(self, icon_size):
        ...
        """
        mlogger.debug(
            "Creating %sx%s bitmap from: %s", icon_size, icon_size, self.icon_file_path
        )
        ...
        bitmap_source = Imaging.BitmapSource.Create(
            scaled_size,
            scaled_size,
            scaled_dpi,
            scaled_dpi,
...
class GenericPyRevitUIContainer(object):
    ...
    def _get_component(self, cmp_name):
        try:
            ...
            raise PyRevitUIError(
                "Can not retrieve item {} from {}".format(cmp_name, self)
            )
    ...
    def _remove_component(self, expired_cmp_name):
        try:
            ...
            raise PyRevitUIError(
                "Can not remove item {} from {}".format(expired_cmp_name, self)
            )
    ...
    def process_deferred(self):
        try:
            ...
            raise PyRevitUIError(
                "Error setting .visible {} | {} ".format(self, visible_err)
            )
        ...
        try:
            ...
            raise PyRevitUIError(
                "Error setting .enabled {} | {} ".format(self, enable_err)
            )
...
class RevitNativeRibbonPanel(GenericRevitNativeUIContainer):
    ...
    def __init__(self, adwnd_ribbon_panel):
        ...
        for adwnd_slideout_item in adwnd_ribbon_panel.Source.SlideOutPanelItemsView:
        ...
        for adwnd_ribbon_item in all_adwnd_ribbon_items:
            try:
                if isinstance(adwnd_ribbon_item, AdWindows.RibbonButton) or isinstance(
                    adwnd_ribbon_item, AdWindows.RibbonToggleButton
                ):
                    self._add_component(RevitNativeRibbonButton(adwnd_ribbon_item))
                elif isinstance(adwnd_ribbon_item, AdWindows.RibbonSplitButton):
                    self._add_component(RevitNativeRibbonGroupItem(adwnd_ribbon_item))
...
class _PyRevitRibbonButton(GenericPyRevitUIContainer):
    def __init__(self, ribbon_button):
        ...
        self.itemdata_mode = isinstance(self._rvtapi_object, UI.RibbonItemData)
    ...
    def set_icon(self, icon_file, icon_size=ICON_MEDIUM):
        try:
            ...
            raise PyRevitUIError(
                "Error in applying icon to button > {} : {}".format(icon_file, icon_err)
            )
    ...
    def set_tooltip(self, tooltip):
        try:
            ...
            raise PyRevitUIError(
                "Item does not have tooltip property: {}".format(tooltip_err)
            )
...
class _PyRevitRibbonComboBox(GenericPyRevitUIContainer):
    ...
    def set_icon(self, icon_file, icon_size=ICON_MEDIUM):
        ...
        try:
            ...
            raise PyRevitUIError(
                "Error in applying icon to ComboBox > {} : {}".format(
                    icon_file, icon_err
                )
            )
    ...
    def current(self, value):
        ...
        if not self.itemdata_mode and hasattr(self._rvtapi_object, "Current"):
            try:
                ...
                raise PyRevitUIError(
                    "Error setting current item: {}".format(current_err)
                )
    ...
    def add_item(self, member_data):
        ...
        if not self.itemdata_mode:
            try:
                ...
                raise PyRevitUIError(
                    "Error adding item to ComboBox: {}".format(add_err)
                )
        return None
    ...
    def add_items(self, member_data_list):
        ...
        if not self.itemdata_mode:
            try:
                ...
                raise PyRevitUIError(
                    "Error adding items to ComboBox: {}".format(add_err)
                )
    ...
    def add_separator(self):
        ...
        if not self.itemdata_mode:
            try:
                ...
                raise PyRevitUIError(
                    "Error adding separator to ComboBox: {}".format(sep_err)
                )
...
class _PyRevitRibbonGroupItem(GenericPyRevitUIContainer):
    ...
    def is_splitbutton(self):
        return isinstance(self._rvtapi_object, UI.SplitButton) or isinstance(
            self._rvtapi_object, UI.SplitButtonData
        )
    ...
    def create_data_items(self):
        ...
        for pyrvt_ui_item in [x for x in self if x.itemdata_mode]:
            ...
            if isinstance(pyrvt_ui_item, _PyRevitRibbonButton):
                rvtapi_ribbon_item = self.get_rvtapi_object().AddPushButton(
                    rvtapi_data_obj
                )
    ...
    def reset_highlights(self):
        if hasattr(AdInternal.Windows, "HighlightMode"):
            adwindows_obj = self.get_adwindows_object()
            if adwindows_obj:
                adwindows_obj.HighlightDropDown = coreutils.get_enum_none(
                    AdInternal.Windows.HighlightMode
                )
    ...
    def highlight_as_new(self):
        if hasattr(AdInternal.Windows, "HighlightMode"):
            adwindows_obj = self.get_adwindows_object()
            if adwindows_obj:
                adwindows_obj.HighlightDropDown = AdInternal.Windows.HighlightMode.New
    ...
    def create_push_button(
            ...
            if update_if_exists:
                ...
                try:
                    ...
                    if button_name != EXEC_PARAMS.command_name:
                        ...
                        if avail_class_name:
                            existing_item.get_rvtapi_object().AvailabilityClassName = (
                                avail_class_name
                            )
        ...
        try:
            button_data = UI.PushButtonData(
                button_name, button_name, asm_location, class_name
            )
            if avail_class_name:
                button_data.AvailabilityClassName = avail_class_name
            if not self.itemdata_mode:
                ribbon_button = self.get_rvtapi_object().AddPushButton(button_data)
            ...
            if not icon_path:
                ...
                try:
                    ...
                    new_button.set_icon(
                        icon_path,
                        icon_size=(
                            ICON_LARGE if self._use_active_item_icon else ICON_MEDIUM
                        ),
                    )
                except PyRevitUIError as iconerr:
                    mlogger.debug(
...
class _PyRevitRibbonPanel(GenericPyRevitUIContainer):
    ...
    def set_panel_background(self, argb_color):
        panel_adwnd_obj = self.get_adwindows_object()
        panel_adwnd_obj.CustomPanelBackground = argb_to_brush(argb_color)

    def set_title_background(self, argb_color):
        panel_adwnd_obj = self.get_adwindows_object()
        panel_adwnd_obj.CustomPanelTitleBarBackground = argb_to_brush(argb_color)

    def set_slideout_background(self, argb_color):
        panel_adwnd_obj = self.get_adwindows_object()
        panel_adwnd_obj.CustomSlideOutPanelBackground = argb_to_brush(argb_color)
    ...
    def add_slideout(self):
        try:
            ...
            raise PyRevitUIError("Error adding slide out: {}".format(slideout_err))
    ...
    def _create_data_items(self):
        ...
        if data_obj_count == 2 or data_obj_count == 3:
            created_rvtapi_ribbon_items = self.get_rvtapi_object().AddStackedItems(
                *rvtapi_data_objs
            )
        # if there is only one item added,
        # add that to panel and forget about stacking
        elif data_obj_count == 1:
            rvtapi_pushbutton = self.get_rvtapi_object().AddItem(*rvtapi_data_objs)
        ...
        for rvtapi_ribbon_item, pyrvt_data_item_name in zip(
            created_rvtapi_ribbon_items, pyrvt_data_item_names
        ):
    ...
    def create_push_button(
        ...
        if self.contains(button_name):
            ...
            mlogger.debug(
                "Parent does not include this button. Creating: %s", button_name
            )
            try:
                button_data = UI.PushButtonData(
                    button_name, button_name, asm_location, class_name
                ...
                if avail_class_name:
                    button_data.AvailabilityClassName = avail_class_name
                if not self.itemdata_mode:
                    ribbon_button = self.get_rvtapi_object().AddItem(button_data)
                    new_button = _PyRevitRibbonButton(ribbon_button)
                else:
                    new_button = _PyRevitRibbonButton(button_data)
    ...
    def _create_button_group(
        ...
        if self.contains(item_name):
            if update_if_exists:
                ...
                raise PyRevitUIError(
                    "Pull down button already exits and "
            ...
            try:
                ...
                if not self.itemdata_mode:
                    mlogger.debug(
                        "Creating pull down button: %s in %s", item_name, self
                    )
                    new_push_button = self.get_rvtapi_object().AddItem(pdbutton_data)
                ...
                raise PyRevitUIError(
                    "Can not create pull down button: {}".format(button_err)
                )

    def create_pulldown_button(self, item_name, icon_path, update_if_exists=False):
        self._create_button_group(
            UI.PulldownButtonData, item_name, icon_path, update_if_exists
        )
...
class _PyRevitRibbonTab(GenericPyRevitUIContainer):
    ...
    def __init__(self, revit_ribbon_tab, is_pyrvt_tab=False):
        ...
        try:
            for revit_ui_panel in HOST_APP.uiapp.GetRibbonPanels(self.name):
                ...
                new_pyrvt_panel = _PyRevitRibbonPanel(
                    revit_ui_panel, self._rvtapi_object
                )
                self._add_component(new_pyrvt_panel)
        except:
            # if .GetRibbonPanels fails, this tab is an existing native tab
            raise PyRevitUIError(
                "Can not get panels for this tab: {}".format(self._rvtapi_object)
            )
    ...
    def check_pyrevit_tab(revit_ui_tab):
        return (
            hasattr(revit_ui_tab, "Tag") and revit_ui_tab.Tag == PYREVIT_TAB_IDENTIFIER
        )
    ...
    def create_ribbon_panel(self, panel_name, update_if_exists=False):
        ...
        if self.contains(panel_name):
            ...
            try:
                ...
                pyrvt_ribbon_panel = _PyRevitRibbonPanel(
                    ribbon_panel, self._rvtapi_object
                )
                pyrvt_ribbon_panel.set_dirty_flag()
                self._add_component(pyrvt_ribbon_panel)

            except Exception as panel_err:
                raise PyRevitUIError("Can not create panel: {}".format(panel_err))
...
class _PyRevitUI(GenericPyRevitUIContainer):
    ...
    def __init__(self, all_native=False):
        ...
        for revit_ui_tab in AdWindows.ComponentManager.Ribbon.Tabs:
            ...
            try:
                if not all_native and _PyRevitRibbonTab.check_pyrevit_tab(revit_ui_tab):
                ...
                mlogger.debug("Tab added to the list of tabs: %s", new_pyrvt_tab.name)
                ...
                self._add_component(new_pyrvt_tab)
                mlogger.debug(
                    "Native tab added to the list of tabs: %s", new_pyrvt_tab.name
                )
    ...
    @staticmethod
    def toggle_ribbon_updator(state, flow_direction=Windows.FlowDirection.LeftToRight):
        # cancel out the ribbon updator from previous runtime version
        current_ribbon_updator = envvars.get_pyrevit_env_var(
        ...
        if state:
            ...
            try:
                ...
                panel_set = main_wnd.FindFirstChild[ribbon_root_type](main_wnd)
            ...
            if panel_set:
                ...
                envvars.set_pyrevit_env_var(
                    envvars.RIBBONUPDATOR_ENVVAR, types.RibbonEventUtils
                )

    def set_RTL_flow(self):
        _PyRevitUI.toggle_ribbon_updator(
            state=True, flow_direction=Windows.FlowDirection.RightToLeft
        )
    ...
    def create_ribbon_tab(self, tab_name, update_if_exists=False):
        if self.contains(tab_name):
            ...
            try:
                ...
                if revit_tab_ctrl:
                    pyrvt_ribbon_tab = _PyRevitRibbonTab(
                        revit_tab_ctrl, is_pyrvt_tab=True
                    )
